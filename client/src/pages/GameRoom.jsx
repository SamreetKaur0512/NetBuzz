...
export default function GameRoom() {
  ...
  useEffect(() => {
    gameAPI.getRoom(roomCode)
      ...
      .catch(() => { toast.error('Room not found'); navigate('/games'); })
  ...
  useEffect(() => {
    ...
    gameSocket.on('gameError', ({ message }) => {
      ...
      const silent = ['Room is full', 'Game already started or finished', 'You are already in this room'];
  ...
  }, [gameSocket, startTimer, timeLimit]);
    ...
    return (
      <div className="game-room">
        ...
        <div className="waiting-room">
          ...
          {/* quiz and puzzle reach here — both support solo */}
          <div style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 24, lineHeight: 1.8 }}>
            Play solo, or share code{' '}
            <strong style={{ color: 'var(--yellow, #FFD700)', fontSize: 15 }}>{roomCode}</strong>
            {' '}with other users to play with them
          </div>

          <div className="players-grid">
            {players.map(p => (
              <div key={p.userId} className={`player-slot${p.ready ? ' ready' : ''}${(p.userId === room?.hostId || p.userId?._id === room?.hostId) ? ' host' : ''}`}>
                <Avatar username={p.username} size={44} />
                <div className="player-slot-name">{p.username}</div>
                <div className="player-slot-tag">
                  {(p.userId === room?.hostId || p.userId?._id === room?.hostId) ? 'Host' : p.ready ? '✓ Ready' : 'Waiting'}
                </div>
              </div>
            ))}
            {/* Empty slots */}
            {[...Array(Math.max(0, (room?.maxPlayers || 8) - players.length))].slice(0, 3).map((_, i) => (
              <div key={`empty-${i}`} className="player-slot" style={{ opacity: 0.3, border: '1px dashed var(--border)' }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'var(--bg-base)', margin: '0 auto' }} />
                <div className="player-slot-name" style={{ color: 'var(--text-muted)' }}>Empty</div>
              </div>
            ))}
          </div>
            ...
            {isHost && (
              <>
                ...
                {players.length < 2 && ['quiz','puzzle','aiquiz','snake'].includes(room?.gameType) && (
                  <div style={{ fontSize: 13, color: 'var(--gold)', fontWeight: 600, marginTop: 4, textAlign: 'center' }}>
                    🎮 Solo Mode — play alone, no win/loss<br/>
                    <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 400 }}>
                      Share the room code to invite others to join.
                    </span>
                  </div>
                )}
                {players.length < 2 && !['quiz','puzzle','aiquiz','snake'].includes(room?.gameType) && (
                  <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>
                    ⚠️ Need at least 2 players to start this game
                  </div>
                )}
              </>
            )}
