...
const getUserById = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, user: { ...userObj, postCount, canSeePosts, followRequestStatus } });
  } catch (err) { next(err); }
};
...
const updateUser = async (req, res, next) => {
  try {
    ...
    if (updates.username) {
      // Enforce uniqueness for display name (case-insensitive)
      const exists = await User.findOne({
        username: { $regex: `^${updates.username.trim()}$`, $options: "i" },
        _id: { $ne: req.params.id },
      });
      if (exists) return res.status(409).json({ success: false, message: "Display name is already taken. Please choose another." });
    }
...
const deleteAccount = async (req, res, next) => {
  try {
    ...
    // 1. Delete all posts + their media files
    ...
    // 2. Remove user from other posts (likes, comments)
    ...
    // 3. Delete all direct messages
    await Message.deleteMany({ $or: [{ senderId: userId }, { receiverId: userId }] });

    // 4. Delete all group messages by this user
    await GroupMessage.deleteMany({ senderId: userId });

    // 5. Handle group memberships
    ...
    // 6. Delete group invites, chat requests, follow requests
