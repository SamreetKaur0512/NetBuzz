...
const replyToComment = async (req, res, next) => {
  try {
    ...
    if (replyId) {
      // Reply to a reply (can be nested)
    ...
    // Populate the new reply - for simplicity, populate up to 5 levels
    ...
    await post.populate("comments.replies.replies.replies.replies.userId", "username userId profilePicture");
    await post.populate("comments.replies.replies.replies.replies.replyToUser", "username");
    await post.populate("comments.replies.replies.replies.replies.replies.userId", "username userId profilePicture");
    await post.populate("comments.replies.replies.replies.replies.replies.replyToUser", "username");

    // Since we just added it, find the last added reply in the tree
    const findLastAddedReply = (replies, userId, text) => {
      for (let i = replies.length - 1; i >= 0; i--) {
        const reply = replies[i];
        if (reply.userId.toString() === userId.toString() && reply.text === text) {
          return reply;
        }
        if (reply.replies && reply.replies.length > 0) {
          const found = findLastAddedReply(reply.replies, userId, text);
          if (found) return found;
        }
      }
      return null;
    ...
    const newReplyPopulated = findLastAddedReply(comment.replies, req.user._id, text.trim());
